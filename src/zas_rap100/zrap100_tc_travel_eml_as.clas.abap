"! @testing BDEF:ZRAP100_R_TravelTP_AS
CLASS zrap100_tc_travel_eml_as DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC
  FOR TESTING
...
CLASS zrap100_tc_travel_eml_as IMPLEMENTATION.

  METHOD class_setup.
    " create the test doubles for the underlying CDS entities
    cds_test_environment = cl_cds_test_environment=>create_for_multiple_cds(
                      i_for_entities = VALUE #(
                        ( i_for_entity = 'ZRAP100_R_TravelTP_AS' ) ) ).
  ...
  METHOD create_with_action.
    ...
    MODIFY ENTITIES OF ZRAP100_R_TravelTP_AS
     ENTITY Travel
    ...
    SELECT * FROM ZRAP100_R_TravelTP_AS INTO TABLE @DATA(lt_travel). "#EC CI_NOWHERE
